...
- Theme system separating brand styling (colors, fonts, logo, QR caption, default legal claim) from label structure — `themes/la-peptides-default.theme.json` and a full white-label example, `themes/white-label-example-true-north.theme.json`.
- Field-mapping system driven entirely by element ids — `field-maps/la-peptides-default.field-map.json` and `field-maps/white-label-example-true-north.field-map.json`.
