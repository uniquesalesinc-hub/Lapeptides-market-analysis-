    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center justify-between">
          <h1 className="font-mono text-xl font-semibold text-white">{invoice.invoiceNumber}</h1>
        ...
        {invoice.quote && (
          <Link href={`/quotes/${invoice.quote.id}`} className="text-sm text-brand-teal">
      ...
      <div className="card p-4 text-sm">
        ...
        <p className="text-white">{PRICE_LIST_LABELS[invoice.priceListCode]}</p>
      </div>
      ...
      {invoice.payments.length > 0 && (
        <section>
          <h2 className="mb-2 font-semibold text-white">Payment History</h2>
          <ul className="space-y-2">
            {invoice.payments.map((p) => (
              <li key={p.id} className="card flex items-center justify-between p-3 text-sm">
                <div>
                  <p className="text-white">{formatMoney(Number(p.amount))}</p>
                  <p className="text-xs text-brand-slate-400">
                    {p.method.replace(/_/g, " ")} · {formatDateTime(p.paidAt)}
                  </p>
                  {p.referenceNote && <p className="text-xs text-brand-slate-400">Ref: {p.referenceNote}</p>}
                </div>
              </li>
            ))}
          </ul>
      ...
      {invoice.achInstructions && (
        <div className="card p-4 text-sm">
          ...
          <p className="text-brand-slate-300">{invoice.achInstructions}</p>
        </div>
      )}
      ...
      {invoice.internalNotes && (
        <div className="card border-brand-warning/30 p-4 text-sm">
          ...
          <p className="text-brand-slate-300">{invoice.internalNotes}</p>
        </div>
      )}
    </div>
  );
